...
public class GetDataObj {
    static String key = "제출용으로 키는 제거하였습니다.";
    ...
    public static int getData(int first, int last){
        //System.out.println(key);
        if (first == 1) clear();

        OkHttpClient cli = new OkHttpClient.Builder().build();
        Request req = new Request.Builder()
                .url("http://openapi.seoul.go.kr:8088/"+key+"/json/TbPublicWifiInfo/"+first+"/"+last+"/")
                .get()
                .build();
        Response res = null;
        ...
        //if (jsonobject.get("TbPublicWifiInfo") == null) return -1;
